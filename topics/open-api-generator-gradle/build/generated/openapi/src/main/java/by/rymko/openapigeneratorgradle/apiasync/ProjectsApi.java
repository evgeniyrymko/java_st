...
public interface ProjectsApi {
    ...
    default CompletableFuture<ResponseEntity<ProjectResponse>> getProjects(@Min(1L)@ApiParam(value = "",required=true) @PathVariable("projectId") Long projectId) {
        return CompletableFuture.supplyAsync(()-> {
            getRequest().ifPresent(request -> {
                for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                    if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
    ...
    @RequestMapping(value = "/projects",
        produces = { "application/json" }, 
        method = RequestMethod.GET)
    default CompletableFuture<ResponseEntity<List<ProjectResponse>>> searchProjects(@ApiParam(value = "find project by name") @Valid @RequestParam(value = "name", required = false) String name) {
        return CompletableFuture.supplyAsync(()-> {
            getRequest().ifPresent(request -> {
                for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                    if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
